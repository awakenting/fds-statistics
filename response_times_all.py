...
#%% plot
resp_plot = df_resp_reso['days'].plot.hist(bins=300,use_index=False,figsize=(18,14))
plt.plot([30,30],[0,500],'r--')
resp_plot.text(30,550, 'Gesetzliche Frist (30 Tage)',ha='center', va='center')
plt.xlim([0,100])
plt.xlabel('Antwortdauer in Tagen')
plt.ylabel('Anzahl der Anfragen')
plt.title('Antwortdauer von ' + str(int(len(df_resp_reso))) + ' abgeschlossenen Anfragen')
plt.savefig(figures_path+'response_times_all.png', bbox_inches='tight',dpi=120)
plt.close()
